...
public class CitizenCreateAppExpanseIT extends BaseBpmnIT {
  ...
  public void init() {
    ...
    headOfficerToken = TestUtils
        .getContent("/json/citizen-create-expanse/headOfficerAccessToken.json");
  ...
  public void happyPath() throws JsonProcessingException {
    ...
    stubDataFactoryRequest(StubData.builder()
        .httpMethod(HttpMethod.POST)
        .headers(Map.of("X-Access-Token", testUserToken))
        .resource("/search/staff-equal-laboratory-id-count")
        .requestBody("{\"laboratoryId\":\"" + labId + "\"}")
        .response("[{\"cnt\":1}]")
        .build());
    ...
    stubDataFactoryRequest(StubData.builder()
        ...
        .headers(Map.of("X-Access-Token", headOfficerToken))
        .resource("/search/application-type-equal-constant-code")
        .requestBody("{\"constantCode\":\"EXPANSE\"}")
        .response(
            "/json/citizen-create-expanse/data-factory/applicationTypeResponse.json")
        .build());
    ...
    var startFormData = deserializeFormData(
        "/json/citizen-create-expanse/form-data/startFormDataActivity.json");
    ...
    completeTask(CompleteActivityDto.builder()
        ...
        .expectedFormData(
            "/json/citizen-create-expanse/form-data/Activity_shared-dispatch-task.json")
        .build());
  ...
  public void testAppAlreadyCreated() throws JsonProcessingException {
    ...
    Assertions.assertThat(errors.get(0)).contains(Map.entry("field", "laboratory"),
        Map.entry("message",
            "Відсутня заява на первинне внесення або створена заява на видалення з Реєстру"),
